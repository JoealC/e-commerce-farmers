...
export const registerBuyer = async(req, res) =>{
    try{
      ...
      if(existingBuyer){
        return errorResponse(res, 401, 'Buyer Already Exists', existingBuyer)
      }
...
export const updateBuyer = async (req, res) => {
    try{
      ...
      successResponse(res, 200, "Updated Buyer Successfull", updateBuyer)
    }catch(err){
      console.log(err)
      errorResponse(res, 500, "Internal Server Error", err)
    }
...
export const orderFarmerProduct = async (req, res) => {
    try {
      ...
      if (product.quantity < quantity) {
        return errorResponse(res, 400, 'Not enough quantity available', quantity);
      }
...
export const listOrders = async(req, res) => {
    try{
        const { buyerId } = req.params   
        if(!buyerId){
          return errorResponse(res, 401, "Buyer Id not found", {})
        }
